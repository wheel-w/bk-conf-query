...
CMDB_API_HOST = os.getenv(
    "CMDB_API_HOST", "https://bkapi.paas-edu.bktencent.com/api/mock-cmdb"
)
...
def _request_cmdb_api(username, api_name, request_method, query_params={}):
    ...
    try:
        ...
        response = requests.request(
            request_method, url, params=query_params, headers=headers
        )
        text = response.text
        ...
        if "count" not in data:
            data = {"count": None, "results": data}
        result = {
            "result": True,
            "data": {"count": data["count"], "info": data["results"]},
        }
    except Exception as e:
        logger.exception(e)
        result = {"result": False, "data": {}, "message": text or str(e)}

    return result


def get_business_info(username, query_params):
    """
    获取cmdb所有业务信息
    """

    result = _request_cmdb_api(username, "api/business_list/", HTTP_GET, query_params)
...
def get_host_info(username, **params):
    ...
    获取主机信息
    """
    result = _request_cmdb_api(username, "api/host_list/", HTTP_GET, params)
